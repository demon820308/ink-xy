...
interface ParsedCharacter {
  ...
  relationships: {
    target: string; // 关联角色的干净姓名（必须是不带任何前缀和括号备注的纯名字，例如："苏晴"）
    type: string; // 关系描述（例如："行动哲学对立，专业上互相信任"）
  }[];
}
\`\`\`
...
2. 多格式自适应提取：输入文本可能包含 Markdown 标题、列表、叙事散文段落或表格。请仔细阅读所有段落与小传，挖掘人物之间的显性/隐性关系，自适应解析出结构化信息。
3. 关系人姓名清洗：在提取 relationships 中的 target 时，必须仅保留干净的纯人名。必须去除任何前缀（如“与”、“对”、“和”）以及中英文小括号及内容（如把 "与苏晴 (闺蜜)"、"苏晴（闺蜜）" 统一清洗为 "苏晴"；把 "与林舒雅 (沈砚妻子)" 清洗为 "林舒雅"）。
4. 人际关系交叉提取：提取每个人物与其他人物之间的关系。
   本小说包含的所有可能角色名单为：${JSON.stringify(nameList)}。
   如果本批角色的描述文本中提及了名单中的任何其他角色，请务必在 \`relationships\` 中建立关系条目。
5. 保持数据完整：不要遗漏任何角色，还原用户输入的所有角色信息。
6. 返回格式：必须直接返回合法的 JSON 数组，绝对不要包含 Markdown 格式的包裹（如 \`\`\`json 标记），不要有任何前导字眼，确保可以直接被 JSON.parse 解析。
7. 属性值内的双引号处理：属性值内部绝对不能包含未转义的英文双引号 (")。如果提取的文本中含有双引号，请替换为中文双引号（“”）或进行正确转义。
8. 属性值内的换行处理：属性值内的所有换行，必须强制使用转义的 \\n，绝对不允许在 JSON 中出现物理换行！
