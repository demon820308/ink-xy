...
export class ReviserAgent extends BaseAgent {
  ...
  async reviseChapter(
    ...
    if (mode === "auto") {
      const templateFilename = resolvedLanguage === "en" ? "reviser_system_en.md" : "reviser_system_zh.md";
      const rawTemplate = PromptLoader.loadRequiredPrompt(templateFilename);

      const isEnglish = resolvedLanguage === "en";

      const routingDirective = autoOutputMode === "rewrite-only"
        ? (isEnglish
        ...
        : autoOutputMode === "patch-only"
          ? (isEnglish
            ...
            : "\n\n分流指令：reviewer 报告 of 阻塞问题属于局部错（措辞、段落形状、疲劳词、信息越界、知识污染）。你必须只输出 PATCHES——不要整章改写。如果做不出补丁，留空 PATCHES。")
          : "";

      const rewriteLengthConstraint = options?.lengthSpec
        ? (isEnglish
            ? `\n  HARD CONSTRAINT: The revised chapter must stay within ${options.lengthSpec.softMin}-${options.lengthSpec.softMax} characters (target: ${options.lengthSpec.target}, ±25%). This is non-negotiable — do not exceed this range.`
            : `\n  硬性约束：重写后的章节必须控制在 ${options.lengthSpec.softMin}-${options.lengthSpec.softMax} 字以内（目标 ${options.lengthSpec.target} 字，±25%）。这是不可突破的底线。`)
        : "";

      const enPatchesDetails = `PATCHES and REVISED_CONTENT serve different problems — choose by problem type, not preference:
...
If Critical issues include both local and whole-chapter problems, use REVISED_CONTENT (it addresses everything in one pass).`;
      ...
      const zhPatchesDetails = `PATCHES 和 REVISED_CONTENT 分别处理不同类型的问题——按问题类型选择，不是按偏好：

PATCHES——处理局部文字问题（措辞、对话、AI痕迹、小的连续性错误）。
  每个 PATCH 引用要修改的原文段落（一句、一段或多段戏皆可），给出替换文本。未涉及的内容保持原样。
...
如果 Critical 同时包含局部问题 and 全章问题，使用 REVISED_CONTENT（一次性解决所有问题）。`;

      const enRevisionPrinciples = `Revision principles:
...
7. Escalate: bad things stack, each worse than the last`;
      ...
      const zhRevisionPrinciples = `修稿原则：
...
7. 坏事叠坏事，每层比上一层过分`;
      ...
      const enCycleAwareRevision = `Cycle-aware revision:
...
- Daily passages that don't serve the main line: rewrite as "bait" — add a detail pointing to the future, a hint, a character reaction that seeds curiosity`;
      ...
      const zhCycleAwareRevision = `小目标周期修稿指引：
- 如果本章应该是"后效"阶段但仍在加压，把最密集的冲突段落改写为展示改变的段落——谁失去了什么、谁的态度变了、新的常态是什么
- 如果本章应该是"爆发"阶段但没有明确兑现，找到最接近回报的场景并放大它——让承诺的释放超过读者预期
- 日常过渡段如果没起到主线引流作用：改写为"钓饵"段——加一条引向未来的线索、暗示、或引起好奇的角色反应`;

      const parts = {
        langPrefix,
        protagonistBlock,
        routingDirective,
        patchesAndRevisedContentDetails: isEnglish ? enPatchesDetails : zhPatchesDetails,
        rewriteLengthConstraint,
        revisionPrinciples: isEnglish ? enRevisionPrinciples : zhRevisionPrinciples,
        cycleAwareRevision: isEnglish ? enCycleAwareRevision : zhCycleAwareRevision,
      };
      ...
      systemPrompt = this.buildLegacySystemPrompt({ langPrefix, gp, protagonistBlock, numericalRule, lengthGuardrail, mode, resolvedLanguage });
    }
